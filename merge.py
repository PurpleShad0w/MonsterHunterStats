...
df_general = pd.DataFrame(data={'Item Name':0,'Quantity Possessed':0,'Item Level':0,'Item Experience':0,'Item Type':0},index=(0,1))
# Fill in the items
for i in range(len(df_items)):
    s = {'Item Name':df_items.iloc[i,1],'Quantity Possessed':df_items.iloc[i,2],'Item Level':0,'Item Experience':0,'Item Type':df_items.iloc[i,5]}
    df_general = df_general.append(s,ignore_index=True)
# Fill in the equipment
for i in range(len(df_equipment)):
    s = {'Item Name':df_equipment.iloc[i,2],'Quantity Possessed':1,'Item Level':df_equipment.iloc[i,3],'Item Experience':df_equipment.iloc[i,4],'Item Type':df_equipment.iloc[i,5]}
    df_general = df_general.append(s,ignore_index=True)
# Fill in the tools
